...
test.describe('Field Email validation', () => {
    ...
    test('Email should contain @', async ({ page }) => {
        await signUpForm.fieldValidation(signUpForm.emailField, 'test.test.test', invalidEmail);
    });

    test('Email shouldn"t contain space', async ({ page }) => {
        await signUpForm.fieldValidation(signUpForm.emailField, 'some name@gmail.com', invalidEmail);
    });

    test('Email should contain a few symbols and a dot after @', async ({ page }) => {
        await signUpForm.fieldValidation(signUpForm.emailField, 'name@gmailcom', invalidEmail);
    });
});
